...
bool contains(char* p,char c)
{
    ...
    {
        if(*(p+i)==c)//Condition to check whether the character of the string is same as the given characteror not 
        t=*(p+i);//Assign the value to new variable
    }
    if(t!='\0')//Conditionto check whether the value of teh variable is null or not
    return true;
    else
    return false;
}
